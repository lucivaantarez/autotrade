...
for (const expected of ['getgenv().scriptkey = getgenv().scriptkey or ""', 'PASTE YOUR ZEKEHUB KEY', 'AutoAcceptTrades = true', 'AutoLeaveAfterTrades = false', 'AccountManager = {', 'task.spawn', 'Poll = 0.4', 'utility.WinterHub', 'ForceSettings', 'force_everyone("trade_requests")', 'force_everyone("give_item_requests")', '_winteraddons.json', 'status = "completed"', 'ts = os.time()', 'count = trade_count', 'items = last_items', 'mine.confirmed and partner.confirmed', 'return loader()']) assert(source.includes(expected));
